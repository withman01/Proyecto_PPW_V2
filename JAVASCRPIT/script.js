...
// Asegúrate de que el SVG tenga id="mapa-ecuador" (en tu HTML ya lo tiene)
...
function boomOnProvince(provEl, gifUrl = "../MULTIMEDIA/imagenes/explosion.t.gif", ms = 900){
  // Caja EN PANTALLA del path (no coords del SVG)
  const rect = provEl.getBoundingClientRect();
  const cx = rect.left + rect.width / 2;
  const cy = rect.top  + rect.height / 2;

  const img = document.createElement("img");
  img.src = gifUrl;          // << PON AQUÍ LA RUTA CORRECTA A TU GIF
